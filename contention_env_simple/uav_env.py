...
class UAVenv(gym.Env):
    ...
    def generate_user_distribution(self, NUM_USER, NUM_UAV, COVERAGE_XY):
        HOTSPOTS = np.array(
            [[250, 300], [700, 250], [400, 750]])  # Position setup in grid size rather than actual distance
        USER_DIS = int((NUM_USER-10)/ NUM_UAV)
        USER_LOC = np.zeros((NUM_USER - 10, 2))
        
        for i in range(len(HOTSPOTS)):
            for j in range(USER_DIS):
                temp_loc_r = random.uniform(-(1/4.5)*COVERAGE_XY, (1/4.5)*COVERAGE_XY)
                temp_loc_theta = random.uniform(0, 2*math.pi)
                temp_loc = self.pol2cart(temp_loc_r, temp_loc_theta)
    ...
    def load_user_distribution(self):
    ...
    #############################################################################
    # def generate_user_rb(self, NUM_USER, COVERAGE_XY):
